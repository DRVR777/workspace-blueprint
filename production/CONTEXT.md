...
5. **Output is consumed downstream.** Files in `04-output/` are consumed by `community/`. Name output files with enough description that a community agent can identify relevance from the filename alone, without reading the spec. Format: `[slug]-v[n]-[brief-description].[ext]`
