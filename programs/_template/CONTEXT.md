...
| Work on a specific program | `programs/[name]/CONTEXT.md` | Relevant contract from `shared/contracts/` |
| Debug a cross-program issue | `shared/contracts/` | Both programs' CONTEXT.md files |
...
- Always check `_planning/adr/` before making architectural choices
- Contracts in `shared/` are the source of truth — if code disagrees, the code is wrong
